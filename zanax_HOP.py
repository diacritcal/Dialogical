...
def seek_plan(state,tasks,plan,depth,verbose=0):
    ...
    # We're done and successful if we end up with an empty task queue. [TLD]
    if tasks == []:
        if verbose > 2: print('depth {} returns plan {}'.format(depth,plan))
        return plan
    # Tasks are expanded left to right so the queue is esentially LIFO. [TLD]
    task1 = tasks[0]
    if task1[0] in operators:
        # A task has 1 or 0 operators encoding its possible state changes. [TLD]
        if verbose > 2: print('depth {} action {}'.format(depth,task1))
        operator = operators[task1[0]]
        # Copy the state so we can restore state if we have to backtrack. [TLD]
        newstate = operator(copy.deepcopy(state),*task1[1:]) # argument_unpacking
        if verbose > 2:
            print('depth {} new state:'.format(depth))
            print_state(newstate)
        if newstate:
            # We add the task to the plan as tasks with operators are actions. [TLD]
            solution = seek_plan(newstate,tasks[1:],plan+[task1],depth+1,verbose)
            if solution != False:
                return solution
    # In this implementation, a task (action) can both have an operator and methods. [TLD]
    if task1[0] in methods:
        if verbose > 2: print('depth {} method instance {}'.format(depth,task1))
        relevant = methods[task1[0]]
        for method in relevant:
            # Each methods returns False or a list of subtasks. Note state is global. [TLD] 
            subtasks = method(state,*task1[1:]) # argument_unpacking
            # You can't just say "if subtasks:", because that's wrong if subtasks == [].
            if verbose > 2:
                print('depth {} new tasks: {}'.format(depth,subtasks))
            if subtasks != False:
                # Add subtasks to front of the queue replacing just completed task. [TLD]
